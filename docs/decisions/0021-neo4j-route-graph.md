...
- **Weighted shortest path via APOC dijkstra / A\*.** Hop-shortest uses the built-in `shortestPath((a)-[:ROUTE*..6]->(b))` (6 hops: no sane itinerary exceeds it, and the bound caps traversal). Min-shortest uses `apoc.algo.dijkstra` over directed `ROUTE>` edges. Km-shortest upgraded to `apoc.algo.aStar` with the haversine heuristic over the latitude/longitude properties — great-circle km never exceeds routed km, so the heuristic is admissible (same optimum, fewer expansions); minutes have no admissible geometric heuristic and stay on dijkstra. The thin-pair integration tests remain the tripwire for both.
...
- The apoc core plugin becomes part of the Neo4j container contract (compose, k8s, Testcontainers all enable it); a Neo4j upgrade must keep apoc.algo.dijkstra and apoc.algo.aStar available.
- Each airport's out-degree is persisted as a node property at seed time (with an idempotent backfill for pre-existing stores), so search ordering and the map payload never re-count edges on the request path.
