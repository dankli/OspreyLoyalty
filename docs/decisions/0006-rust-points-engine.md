...
- The engine logs JSON and echoes correlation ids. The Phase 6 observability work added a Prometheus `/metrics` endpoint and OTLP-ready wiring so it is scraped like the rest of the fleet and appears on the RED dashboard, even though it deliberately stays outside the earn path.
