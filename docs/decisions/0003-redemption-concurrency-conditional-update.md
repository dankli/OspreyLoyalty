...
- The compensation itself has a crash window: if the process dies between the failed burn insert and the compensating increment, that decrement is lost — the same projection-gap class as above, with the same ledger-sum production answer. A same-key retry racing its original can also surface as a 400 once (the retry misses the fast-path, then finds the balance already decremented by the original), and heals to `alreadyApplied: true` on the next retry.
