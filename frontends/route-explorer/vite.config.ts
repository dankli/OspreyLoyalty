...
// when the quotes match (single-CSS-bundle apps, which every portal in this repo is).
...
export default defineConfig({
  plugins: [
    svelte(),
    svelteTesting(),
    // ADR-0004/0022: expose the mount contract for the shell. Additive — standalone dev/build keep working.
    federation({
      name: "routeExplorer",
      ...
      // No `shared`: no other remote uses Svelte, and the framework-less shell deduplicates nothing anyway.
    }),
