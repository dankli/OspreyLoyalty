...
- **Weighted shortest path without APOC.** Hop-shortest uses the built-in `shortestPath((a)-[:ROUTE*..6]->(b))` (6 hops: no sane itinerary exceeds it, and the bound caps traversal). Km/min-shortest first finds the hop-shortest length `L`, then enumerates paths of length `L..L+1` and picks the minimum `reduce`-sum. The `L..L+1` window catches the common "one extra hop is shorter in km" case while staying tractable hub-to-hub; a 2-second transaction timeout is the hard backstop. If the heuristic ever proves too narrow, `apoc.algo.dijkstra` (APOC core plugin) is the documented escape hatch — a container config change, not a code rewrite.
...
- The `L..L+1` weighted-path heuristic can miss a cheaper path that is ≥2 hops longer than the hop-optimal route. Accepted for v1; the APOC fallback covers the day it matters.
