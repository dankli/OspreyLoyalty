...
flowchart TD
    ...
    subgraph Services
        ...
        Members["members<br/>(C# / .NET 8)<br/>core domain<br/>:5080"]
    ...
    subgraph Infra
        ...
        Grafana["Grafana<br/>RED dashboards"]
    end
    ...
    MemberPortal -->|"GraphQL<br/>X-Correlation-Id"| Gateway
    AdminPortal -->|"REST (direct, demo CORS)<br/>X-Correlation-Id"| Members
    AdminPortal -->|"REST (direct, demo CORS)"| Partners

    Gateway -->|"REST, 2 s timeout<br/>X-Correlation-Id"| Members
    Gateway -->|"REST, 2 s timeout"| Partners

    Partners -->|"EarnEvent (JSON)<br/>correlationId in the payload"| RabbitMQ
...
**Grafana** — pre-provisioned RED dashboards (request rate, error rate, duration) per service. Reads from Prometheus; available at `localhost:3000` in the compose stack.
