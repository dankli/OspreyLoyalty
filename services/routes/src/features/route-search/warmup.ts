...
import { DIJKSTRA_QUERY, HOP_SHORTEST_QUERY } from "./cypher.js";
...
export async function warmRouteGraph(driver: Driver, log: pino.Logger): Promise<void> {
  ...
  for (const weight of ["km", "min"] as const) {
    await readQuery(driver, DIJKSTRA_QUERY, { ...WARM_PAIR, weight }, WARM_TIMEOUT_MS);
  }
