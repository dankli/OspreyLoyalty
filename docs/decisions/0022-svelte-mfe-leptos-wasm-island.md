...
- **The JS↔WASM boundary is typed arrays and indices, not JSON.** The exported surface is a single `RouteMap` handle: `new(host, lats: &[f32], lons: &[f32], degrees: &[u32], labels: Vec<String>, on_pick)` — degrees size the hub dots, labels are the IATA codes drawn at deep zoom — `draw_base()`, `highlight_destinations(from: u32, dests: &[u32])`, `show_path(path: &[u32])`, `zoom_in()`, `zoom_out()`, `reset_view()`. Pointer interaction lives inside the island (click hit-testing, wheel zoom at the cursor, drag-to-pan); a pick surfaces in JS via the `on_pick` callback carrying the airport *index*. Svelte fetches the airport list once, builds `Float32Array`s, and keeps all metadata (IATA codes, names, tooltips) on the JS side; indices are the shared currency. Svelte never sees Leptos.
- **No user-facing text inside the island.** The status line and all controls' labels render on the Svelte side, so the ADR-0009 five-language catalogs cover the whole remote; the crate stays free of copy and of locale plumbing.
- **An embedded basemap, not a tile server.** The island carries Natural Earth data (public domain): 110m land polygons for coastlines and 50m populated places for city labels, packed by `wasm-map/tools/build-basemap.mjs` into ~65 KB of compile-time-embedded binary. The zoom level gates which scaleranks get labelled (whole world → megacities only; zoomed in → everything), and a greedy screen-space declutter lets important names win. Geographic names are map *data* like the coordinates — not UI copy — so they live in the island without contradicting the no-text rule.
