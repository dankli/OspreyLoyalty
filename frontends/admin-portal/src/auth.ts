// Browser OIDC (authorization code + PKCE) for the admin portal, gated by VITE_AUTH_ENABLED so the
// default build, the e2e and every test run exactly as before (no login, full access). When enabled
// the shell and both portals share one access token via a single sessionStorage key (SSO across the
// module-federation boundary — ADR-0007). Admin actions additionally require the `admin` role.
...
/** With auth off the console is fully usable (demo/dev); with it on, admin actions need the role. */
...
/** Gate the app on a valid session; no-op when auth is disabled. See member-portal/src/auth.ts. */
