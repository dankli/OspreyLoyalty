...
- **Secrets as a pattern, not secrecy.** Datastore connection strings now flow through the `osprey-datastores` Secret (`secrets.yaml` → `secretKeyRef` in the deployments), so a real environment swaps ONE manifest for an ExternalSecret/CSI mount without touching deployments. The committed values are demo-public in-cluster hostnames; treat the object shape, not the file, as the takeaway.
